...
export function registerGetMatchPlayerItemsTool(server: McpServer, deadlockClient: DeadlockClient): void {
  server.registerTool(
    "get_match_player_items",
    {
      title: "Get Match Player Items",
      description: "Get your build timeline and final items for a match",
      inputSchema: {
    ...
    async ({ account_id, match_id }) => {
      ...
      const result = await deadlockClient.getPlayerMatchItems(resolvedAccountId, match_id);
      return {
        content: [
          {
            type: "text",
